// TimeseriesUI is a standalone web UI server for InfluxDB 1.x compatible databases.
//
// It serves the TimeseriesUI web interface and proxies API requests (/query, /write,
// /ping, /debug/*) to remote InfluxDB instances. Connections are managed directly
// in the UI, allowing you to switch between multiple InfluxDB servers.
//
// Usage:
//
//	timeseriesui
...
func main() {
	...
	flag.IntVar(&port, "port", 8080, "Port for the UI server to listen on")
	flag.Parse()
	...
	fmt.Printf("TimeseriesUI starting on http://localhost%s/ui/\n", addr)
	if defaultURL != "" {
